    ...
    <div class="card-header">
        <div class="row">
            ...
            <div class="col-sm-6">
                <div class="pull-left">
                    <br>
                    <a href="{{route('maquinariapdf')}}" class="btn btn-info"><i class="zmdi zmdi-file"></i> Reporte</a>
                </div>
                <div class="pull-right">
                    <br>
                    <a href="{{route('maquinarias.create')}}" class="btn btn-info"><i class="zmdi zmdi-plus"></i> Nuevo Registro</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <table id="tabla" class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">{{$maquinaria->modelo}}</td>
                    <td class="text-center">{{$maquinaria->color}}</td>
                    <td class="text-center">{{$maquinaria->imagen}}</td>
                    <td class="text-center">{{$maquinaria->activo}}</td>
                    <td  class="text-center">
                    </td>
                </tr>
                    @include('panel.maquinarias.show')
                    @include('panel.maquinarias.delete')
                    @include('panel.maquinarias.estado')
                @endforeach
            ...
            formatters: {
                ...
                "commands": function(column, row) {
                    if(row.activo==1)
                    {
                        ...
                        var edit="<a href=\"/panel/maquinarias/"+row.id+"/edit\" class=\"btn btn-icon command-edit waves-effect waves-circle\"><span style=\"color: #00bcd4;\" class=\"zmdi zmdi-edit\"></span></a> " ;
                ...
                "imagen": function(column,row){
                    return "<img src=\"{{asset('imagenes')}}/maquinarias/" +row.imagen+"\" width=\"50px\" height=\"50px\" class=\"img-responsive img-thumbnail\" data-row-id=\"" + row.id + "\">";
                },
